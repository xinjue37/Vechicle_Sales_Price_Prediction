...
|**4. Exploratory Data Analysis (EDA)**|<ul><li>To find out some possible useful insights from the features</li><li>just perform simple EDA due to time constraints</li></ul>|
...
| **6. Encoding Categorical Data** | Try different encoding methods: <ul><li>One-hot Encoding</li><li>Target Encoding</li><li>Feature Hashing</li><li>Neural Network Embedding (excluded due to cost)</li></ul> |
| **7. Model Building & Training** | Use the following models (no need for normalization or null handling): <ul><li>XGBRegressor</li><li>CatBoostRegressor</li><li>HistGradientBoostingRegressor</li></ul> |
| **8. Model Evaluation** | Perform slice-based evaluation by the `Make` feature to analyze model performance across car brands. |
| **9. Feature Importance Analysis** | Analyze and compare which features are most important to each model. |
